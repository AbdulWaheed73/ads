...
import { Fingerprint, Copy, Trash2 } from "lucide-react";

// Inline MD5 implementation (public domain)
function md5(input: string): string {
  ...
  function rstrMD5(s: string) {
    ...
    const bin = binlMD5(x, s.length * 8);
    let rstr = "";
    for (let i = 0; i < bin.length * 32; i += 8) rstr += String.fromCharCode((bin[i >> 5] >>> i % 32) & 0xff);
    return rstr;
  }
            ...
            {results.map((r) => (
              <div key={r.algorithm} className="space-y-1">
                <div className="flex items-center justify-between">
                  <Badge variant="secondary">{r.algorithm}</Badge>
                  <Button
                    variant="ghost"
                    size="sm"
                    onClick={() => {
                      navigator.clipboard.writeText(r.hash);
                      toast.success(`${r.algorithm} hash copied!`);
                    }}
                  >
                ...
                <div className="rounded-md border bg-muted/30 px-3 py-2">
                  <code className="text-xs font-mono break-all">{r.hash}</code>
                </div>
              </div>
            ))}
          </div>
